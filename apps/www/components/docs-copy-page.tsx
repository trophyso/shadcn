...
import {
  ...
  DropdownMenuItem,
  DropdownMenuTrigger,
} from "@/registry/trophy/ui/dropdown-menu"
import {
  Popover,
  PopoverAnchor,
  PopoverContent,
  PopoverTrigger,
} from "@/registry/trophy/ui/popover"
        ...
        >
          {Object.entries(menuItems).map(([key, value]) => (
            <Button
              ...
              asChild
              key={key}
              className="*:[svg]:text-muted-foreground h-10 w-full justify-start px-2 text-base font-normal"
            >
              {value(url)}
            </Button>
          ))}
